...
;; Package-Version: 20210508.836
;; Package-Commit: fb685e1e43db979e25713081d8ae4073453bbd5e
;; Version: 0.0.1
...
;; Customizations
(defgroup undo-tree-sidebar nil
  "A major mode leveraging `undo-tree-sidebar' to display buffers in a sidebar."
  :group 'convenience)

(defcustom undo-tree-sidebar-use-custom-modeline t
  "Show `undo-tree-sidebar' with custom modeline.

This uses format specified by `undo-tree-sidebar-mode-line-format'."
  :type 'boolean
  :group 'undo-tree-sidebar)

(defcustom undo-tree-sidebar-mode-line-format
  '("%e" mode-line-front-space
    mode-line-buffer-identification
    " "  mode-line-end-spaces)
  "Mode line format for `undo-tree-sidebar'."
  :type 'list
  :group 'undo-tree-sidebar)

(defcustom undo-tree-sidebar-display-column-titles nil
  "Whether or not to display the column titles in sidebar."
  :type 'boolean
  :group 'undo-tree-sidebar)

(defcustom undo-tree-sidebar-display-summary nil
  "Whether or not to display summary in sidebar."
  :type 'boolean
  :group 'undo-tree-sidebar)

(defcustom undo-tree-sidebar-width 35
  "Width of the `undo-tree-sidebar' buffer."
  :type 'integer
  :group 'undo-tree-sidebar)

(defcustom undo-tree-sidebar-pop-to-sidebar-on-toggle-open t
  "Whether to jump to sidebar upon toggling open.

This is used in conjunction with `undo-tree-sidebar-toggle-sidebar'."
  :type 'boolean
  :group 'undo-tree-sidebar)

(defcustom undo-tree-sidebar-use-custom-font nil
  "Show `undo-tree-sidebar' with custom font.

This face can be customized using `undo-tree-sidebar-face'."
  :type 'boolean
  :group 'undo-tree-sidebar)

(defcustom undo-tree-sidebar-face nil
  "Face used by `undo-tree-sidebar' for custom font.

This only takes effect if `undo-tree-sidebar-use-custom-font' is true."
  :type 'list
  :group 'undo-tree-sidebar)
...
(define-derived-mode undo-tree-sidebar-mode undo-tree-mode
  "Ibuffer-sidebar"
  "A major mode that puts `undo-tree' in a sidebar."
  :group 'undo-tree-sidebar
  (let ((inhibit-read-only t))
    (setq window-size-fixed 'width)

    (when undo-tree-sidebar-use-custom-font
      (undo-tree-sidebar-set-font))

    ;; Remove column titles.
    (unless undo-tree-sidebar-display-column-titles
      (advice-add 'undo-tree-update-title-and-summary
                  :after 'undo-tree-sidebar-remove-column-headings))

    ;; Hide summary.
    ;(unless undo-tree-sidebar-display-summary
      ;(setq-local undo-tree-display-summary nil))
    ...
    (let ((window (get-buffer-window buffer)))
      ...
      (with-selected-window window
        (let ((window-size-fixed))
          ...
          (window-resize window -13))))  ;;;CUSTOM
...
(defun ibuffer-sidebar-get-or-create-buffer ()  ;;;HERE!!
