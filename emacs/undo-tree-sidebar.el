...
(eval-and-compile
  (with-no-warnings
    (if (version< emacs-version "26")
        (progn
          (defalias 'undo-tree-sidebar-if-let* #'if-let)
          (defalias 'undo-tree-sidebar-when-let* #'when-let)
          (function-put #'undo-tree-sidebar-if-let* 'lisp-indent-function 2)
          (function-put #'undo-tree-sidebar-when-let* 'lisp-indent-function 1))
      (defalias 'undo-tree-sidebar-if-let* #'if-let*)
      (defalias 'undo-tree-sidebar-when-let* #'when-let*))))



(defcustom undo-tree-sidebar-name " *undo-tree-sidebar*"
  "The name of `-sidebar' buffer."
  :type 'string
  :group 'ibuffer-sidebar)



(defun undo-tree-sidebar-get-or-create-buffer ()  ;;;HERE!!
  "Get or create a `undo-tree-sidebar' buffer."
  ;;(let ((name undo-tree-sidebar-name))
  (let ((name undo-tree-sidebar-name))
    (undo-tree-sidebar-if-let* ((existing-buffer (get-buffer name)))
        existing-buffer
      (let ((new-buffer (generate-new-buffer name)))
        (with-current-buffer new-buffer
          (undo-tree-sidebar-setup))
        new-buffer))))

(defun undo-tree-sidebar-setup ()
  "Bootstrap `ibuffer-sidebar'.

Sets up both `ibuffer' and `ibuffer-sidebar'."
;  (ibuffer-mode)
 ; (ibuffer-update nil)
  (run-hooks 'undo-tree-mode-hook))



(defun undo-tree-sidebar-visualize ()
  ...
  (let ((undo-tree buffer-undo-tree)  ;;;;HERE!!
        (buff (current-buffer))
	(display-buffer-mark-dedicated 'soft))
    
    (switch-to-buffer-other-window
     ;;(get-buffer-create undo-tree-visualizer-buffer-name))
     (undo-tree-sidebar-get-or-create-buffer))
